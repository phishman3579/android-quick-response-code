...
package com.jwetherell.quick_response_code.core.common.detector;
...
/**
 * <p>
 * A somewhat generic detector that looks for a barcode-like rectangular region within an image. It
 * looks within a mostly white region of an image for a region of black and white, but mostly black.
...
public final class MonochromeRectangleDetector {
     ...
     *         last points are opposed on the diagonal, as are the second and third. The first point
     *         will be
     *         the topmost point and the last, the bottommost. The second point will be leftmost and
     *         the
     *         third, the rightmost
    ...
    public ResultPoint[] detect() throws NotFoundException {
        ...
        ResultPoint pointA = findCornerFromCenter(halfWidth, 0, left, right, halfHeight, -deltaY,
                top, bottom, halfWidth >> 1);
        top = (int) pointA.getY() - 1;
        ResultPoint pointB = findCornerFromCenter(halfWidth, -deltaX, left, right, halfHeight, 0,
                top, bottom, halfHeight >> 1);
        left = (int) pointB.getX() - 1;
        ResultPoint pointC = findCornerFromCenter(halfWidth, deltaX, left, right, halfHeight, 0,
                top, bottom, halfHeight >> 1);
        ...
        pointA = findCornerFromCenter(halfWidth, 0, left, right, halfHeight, -deltaY, top, bottom,
                halfWidth >> 2);
    ...
    private ResultPoint findCornerFromCenter(int centerX, int deltaX, int left, int right,
        ...
        for (int y = centerY, x = centerX; y < bottom && y >= top && x < right && x >= left; y += deltaY, x += deltaX) {
            ...
            if (range == null) {
                if (lastRange == null) { throw NotFoundException.getNotFoundInstance(); }
                // lastRange was found
                if (deltaX == 0) {
                    int lastY = y - deltaY;
                    ...
                    if (lastRange[0] < centerY) {
                        if (lastRange[1] > centerY) { return new ResultPoint(lastX,
    ...
    /**
     * Computes the start and end of a region of pixels, either horizontally or vertically, that
     * could
     * be part of a Data Matrix barcode.
     * 
     * @param fixedDimension if scanning horizontally, this is the row (the fixed vertical location)
     *            where we are scanning. If scanning vertically it's the column, the fixed
     *            horizontal location
            ...
            if (horizontal ? image.get(start, fixedDimension) : image.get(fixedDimension, start)) {
                ...
                do {
                    start--;
                        ...
                        && !(horizontal ? image.get(start, fixedDimension) : image.get(
        ...
        while (end < maxDim) {
            if (horizontal ? image.get(end, fixedDimension) : image.get(fixedDimension, end)) {
                ...
                do {
                        ...
                        && !(horizontal ? image.get(end, fixedDimension) : image.get(
                                fixedDimension, end)));
