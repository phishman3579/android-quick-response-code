...
import com.jwetherell.quick_response_code.core.Binarizer;
...
public final class HybridBinarizer extends GlobalHistogramBinarizer {
    ...
    private static void threshold8x8Block(byte[] luminances, int xoffset, int yoffset,
            int threshold, int stride, BitMatrix matrix) {
        for (int y = 0, offset = yoffset * stride + xoffset; y < BLOCK_SIZE; y++, offset += stride) {
            for (int x = 0; x < BLOCK_SIZE; x++) {
                // Comparison needs to be <= so that black == 0 pixels are black even if the
                // threshold is 0.
        ...
        for (int y = 0; y < subHeight; y++) {
            ...
            for (int x = 0; x < subWidth; x++) {
                ...
                if (max - min <= 24) {
                    ...
                    if (y > 0 && x > 0) {
                        // Correct the "white background" assumption for blocks that have neighbors
                        // by comparing
                        // the pixels in this block to the previously calculated black points. This
                        // is based on
                        // the fact that dark barcode symbology is always surrounded by some amount
                        // of light
                        // background for which reasonable black point estimates were made. The bp
                        // estimated at
                        // the boundaries is used for the interior.

                        // The (min < bp) is arbitrary but works better than other heuristics that
                        // were tried.
