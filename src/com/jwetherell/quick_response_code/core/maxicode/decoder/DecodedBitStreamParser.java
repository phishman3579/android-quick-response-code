...
package com.jwetherell.quick_response_code.core.maxicode.decoder;
...
/**
 * <p>
 * MaxiCodes can encode text or structured information as bits in one of several modes, with
 * multiple character sets in one code. This class decodes the bits back into text.
 * </p>
 * 
 * @author mike32767
...
final class DecodedBitStreamParser {
    ...
    private static final String[] SETS = {
            ...
            "`abcdefghijklmnopqrstuvwxyz" + ECI + FS + GS + RS + NS + '{' + PAD
                    + "}~\u007F;<=>?[\\]^_ ,./:@!|" + PAD + TWOSHIFTA + THREESHIFTA + PAD + SHIFTA
                    + SHIFTC + SHIFTD + SHIFTE + LATCHA,
        ...
        switch (mode) {
            ...
            case 3:
                ...
                if (mode == 2) {
                    ...
                    NumberFormat df = new DecimalFormat("0000000000".substring(0,
                            getPostCode2Length(bytes)));
    ...
    private static String getPostCode3(byte[] bytes) {
        return String.valueOf(new char[] {
                SETS[0].charAt(getInt(bytes, new byte[] {
                        39, 40, 41, 42, 31, 32
                })), SETS[0].charAt(getInt(bytes, new byte[] {
                        33, 34, 35, 36, 25, 26
                })), SETS[0].charAt(getInt(bytes, new byte[] {
        ...
        for (int i = start; i < start + len; i++) {
            ...
            switch (c) {
                ...
                case THREESHIFTA:
                    ...
                    break;
                case NS:
                    int nsval = (bytes[++i] << 24) + (bytes[++i] << 18) + (bytes[++i] << 12)
                            + (bytes[++i] << 6) + bytes[++i];
