...
package com.jwetherell.quick_response_code.core.oned.rss;
...
public final class RSS14Reader extends AbstractRSSReader {
        ...
        for (Pair left : possibleLeftPairs) {
            if (left.getCount() > 1) {
                for (Pair right : possibleRightPairs) {
                    if (right.getCount() > 1) {
                        if (checkChecksum(left, right)) { return constructResult(left, right); }
                    }
                }
            }
        }
    ...
    private static Result constructResult(Pair leftPair, Pair rightPair) {
        ...
        return new Result(String.valueOf(buffer.toString()), null, new ResultPoint[] {
    ...
    private Pair decodePair(BitArray row, boolean right, int rowNumber, Map<DecodeHintType, ?> hints) {
        try {
            ...
            return new Pair(1597 * outside.getValue() + inside.getValue(),
                    outside.getChecksumPortion() + 4 * inside.getChecksumPortion(), pattern);
        } catch (NotFoundException re) {
            return null;
        }
    }

    private DataCharacter decodeDataCharacter(BitArray row, FinderPattern pattern,
        ...
        if (outsideChar) {
            if ((oddSum & 0x01) != 0 || oddSum > 12 || oddSum < 4) { throw NotFoundException
                    .getNotFoundInstance(); }
            ...
            if ((evenSum & 0x01) != 0 || evenSum > 10 || evenSum < 4) { throw NotFoundException
                    .getNotFoundInstance(); }
        ...
        for (int x = rowOffset; x < width; x++) {
            if (row.get(x) ^ isWhite) {
                ...
                if (counterPosition == 3) {
                    if (isFinderPattern(counters)) { return new int[] {
                            patternStart, x
                    }; }
