...
final class FieldParser {
    ...
    static String parseFieldsInGeneralPurpose(String rawInformation) throws NotFoundException {
        if (rawInformation.length() == 0) { return null; }

        // Processing 2-digit AIs

        if (rawInformation.length() < 2) { throw NotFoundException.getNotFoundInstance(); }
        ...
        for (Object[] dataLength : TWO_DIGIT_DATA_LENGTH) {
            if (dataLength[0].equals(firstTwoDigits)) {
                if (dataLength[1] == VARIABLE_LENGTH) { return processVariableAI(2,
                        (Integer) dataLength[2], rawInformation); }
                return processFixedAI(2, (Integer) dataLength[1], rawInformation);
            }
        }

        if (rawInformation.length() < 3) { throw NotFoundException.getNotFoundInstance(); }

        String firstThreeDigits = rawInformation.substring(0, 3);

        for (Object[] dataLength : THREE_DIGIT_DATA_LENGTH) {
            if (dataLength[0].equals(firstThreeDigits)) {
                if (dataLength[1] == VARIABLE_LENGTH) { return processVariableAI(3,
                        (Integer) dataLength[2], rawInformation); }
        ...
        for (Object[] dataLength : THREE_DIGIT_PLUS_DIGIT_DATA_LENGTH) {
            if (dataLength[0].equals(firstThreeDigits)) {
                if (dataLength[1] == VARIABLE_LENGTH) { return processVariableAI(4,
                        (Integer) dataLength[2], rawInformation); }
                return processFixedAI(4, (Integer) dataLength[1], rawInformation);
            }
        }

        if (rawInformation.length() < 4) { throw NotFoundException.getNotFoundInstance(); }
    ...
    private static String processFixedAI(int aiSize, int fieldSize, String rawInformation)
        ...
        if (rawInformation.length() < aiSize) { throw NotFoundException.getNotFoundInstance(); }

        String ai = rawInformation.substring(0, aiSize);

        if (rawInformation.length() < aiSize + fieldSize) { throw NotFoundException
                .getNotFoundInstance(); }
