...
package com.jwetherell.quick_response_code.core.oned.rss.expanded.decoders;
...
final class GeneralAppIdDecoder {
    ...
    private boolean isStillNumeric(int pos) {
        ...
        for (int i = pos; i < pos + 3; ++i) {
            if (this.information.get(i)) { return true; }
        }
    ...
    private DecodedNumeric decodeNumeric(int pos) {
        if (pos + 7 > this.information.getSize()) {
            ...
            if (numeric == 0) { return new DecodedNumeric(this.information.getSize(),
    ...
    DecodedInformation decodeGeneralPurposeField(int pos, String remaining) {
        ...
        if (lastDecoded != null && lastDecoded.isRemaining()) { return new DecodedInformation(
                this.current.getPosition(), this.buffer.toString(), lastDecoded.getRemainingValue()); }
    ...
    private BlockParsedResult parseNumericBlock() {
        while (isStillNumeric(current.getPosition())) {
            ...
            if (numeric.isSecondDigitFNC1()) {
                DecodedInformation information = new DecodedInformation(current.getPosition(),
                        buffer.toString());
                return new BlockParsedResult(information, true);
            }
    ...
    private DecodedChar decodeIsoIec646(int pos) {
        ...
        if (fiveBitValue >= 5 && fiveBitValue < 15) { return new DecodedChar(pos + 5,
                (char) ('0' + fiveBitValue - 5)); }

        int sevenBitValue = extractNumericValueFromBitArray(pos, 7);

        if (sevenBitValue >= 64 && sevenBitValue < 90) { return new DecodedChar(pos + 7,
                (char) (sevenBitValue + 1)); }

        if (sevenBitValue >= 90 && sevenBitValue < 116) { return new DecodedChar(pos + 7,
                (char) (sevenBitValue + 7)); }
        ...
        switch (eightBitValue) {
            ...
            case 252:
                ...
                break;
            default:
                throw new IllegalArgumentException("Decoding invalid ISO/IEC 646 value: "
                        + eightBitValue);
        }
    ...
    private DecodedChar decodeAlphanumeric(int pos) {
        ...
        if (sixBitValue >= 32 && sixBitValue < 58) { return new DecodedChar(pos + 6,
                (char) (sixBitValue + 33)); }
    ...
    private boolean isAlphaOr646ToNumericLatch(int pos) {
        ...
        for (int i = pos; i < pos + 3; ++i) {
            if (this.information.get(i)) { return false; }
        }
        return true;
    }
    ...
    private boolean isNumericToAlphaNumericLatch(int pos) {
        ...
        for (int i = 0; i < 4 && i + pos < this.information.getSize(); ++i) {
            if (this.information.get(pos + i)) { return false; }
        }
        return true;
    }
}
