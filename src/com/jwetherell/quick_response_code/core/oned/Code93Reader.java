...
public final class Code93Reader extends OneDReader {
    ...
    public Result decodeRow(int rowNumber, BitArray row, Map<DecodeHintType, ?> hints)
        ...
        if (nextStart == end || !row.get(nextStart)) { throw NotFoundException
                .getNotFoundInstance(); }
        ...
        for (int i = rowOffset; i < width; i++) {
            if (row.get(i) ^ isWhite) {
                ...
                if (counterPosition == patternLength - 1) {
                    if (toPattern(counters) == ASTERISK_ENCODING) { return new int[] {
                            patternStart, i
                    }; }
    ...
    private static int toPattern(int[] counters) {
        ...
        for (int i = 0; i < max; i++) {
            ...
            if (scaledUnshifted < 1 || scaledUnshifted > 4) { return -1; }
            if ((i & 0x01) == 0) {
    ...
    private static void checkOneChecksum(CharSequence result, int checkPosition, int weightMax)
        ...
        if (result.charAt(checkPosition) != ALPHABET[total % 47]) { throw ChecksumException
                .getChecksumInstance(); }
    }
