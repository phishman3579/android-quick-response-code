...
package com.jwetherell.quick_response_code.core.oned;
...
import com.jwetherell.quick_response_code.core.Reader;
...
public abstract class OneDReader implements Reader {
     ...
     * rowStep is bigger as the image is taller, but is always at least 1. We've somewhat
     * arbitrarily
    ...
    private Result doDecode(BinaryBitmap image, Map<DecodeHintType, ?> hints)
        ...
        for (int x = 0; x < maxLines; x++) {
            ...
            // While we have the image data in a BitArray, it's fairly cheap to reverse it in place
            // to
            // handle decoding upside down barcodes.
            for (int attempt = 0; attempt < 2; attempt++) {
                if (attempt == 1) { // trying again?
                    row.reverse(); // reverse the row and continue
                    // This means we will only ever draw result points *once* in the life of this
                    // method
                ...
                try {
                    ...
                    if (attempt == 1) {
                        ...
                        ResultPoint[] points = result.getResultPoints();
                        if (points != null) {
                            points[0] = new ResultPoint(width - points[0].getX() - 1,
                                    points[0].getY());
                            points[1] = new ResultPoint(width - points[1].getX() - 1,
                                    points[1].getY());
                        }
                    }
                    return result;
    ...
    /**
     * Records the size of successive runs of white and black pixels in a row, starting at a given
     * point.
     * The values are recorded in the given array, and the number of runs recorded is equal to the
     * size
     * of the array. If the row starts on a white pixel at the given start point, then the first
     * count
     * recorded is the run of white pixels starting from that point; likewise it is the count of a
     * run
    ...
    protected static void recordPattern(BitArray row, int start, int[] counters)
        ...
        int end = row.getSize();
        if (start >= end) { throw NotFoundException.getNotFoundInstance(); }
        ...
        if (!(counterPosition == numCounters || (counterPosition == numCounters - 1 && i == end))) { throw NotFoundException
                .getNotFoundInstance(); }
    }
    ...
    protected static void recordPatternInReverse(BitArray row, int start, int[] counters)
        ...
        if (numTransitionsLeft >= 0) { throw NotFoundException.getNotFoundInstance(); }
     ...
     *         where the ratio has been multiplied by 256. So, 0 means no variance (perfect match);
     *         256 means
     *         the total variance between counters and patterns equals the pattern length, higher
     *         values mean
     *         even more variance
     */
    protected static int patternMatchVariance(int[] counters, int[] pattern,
        ...
        for (int x = 0; x < numCounters; x++) {
            ...
            int variance = counter > scaledPattern ? counter - scaledPattern : scaledPattern
                    - counter;
            if (variance > maxIndividualVariance) { return Integer.MAX_VALUE; }
