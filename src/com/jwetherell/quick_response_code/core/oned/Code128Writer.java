...
public final class Code128Writer extends UPCEANWriter {
    ...
    public BitMatrix encode(String contents, BarcodeFormat format, int width, int height,
        ...
        if (format != BarcodeFormat.CODE_128) { throw new IllegalArgumentException(
    ...
    public byte[] encode(String contents) {
        ...
        // Check length
        if (length < 1 || length > 80) { throw new IllegalArgumentException(
    ...
    private static boolean isDigits(CharSequence value, int start, int length) {
        ...
        for (int i = start; i < end && i < last; i++) {
            ...
            if (c < '0' || c > '9') {
                if (c != ESCAPE_FNC_1) { return false; }
                end++; // ignore FNC_1
            }
        }
