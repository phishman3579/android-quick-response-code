...
public final class Code128Reader extends OneDReader {
    ...
    public Result decodeRow(int rowNumber, BitArray row, Map<DecodeHintType, ?> hints)
            ...
            switch (codeSet) {
                ...
                case CODE_CODE_A:
                    if (code < 64) {
                        ...
                        // Don't let CODE_STOP, which always appears, affect whether whether we
                        // think the last
        ...
        // Check for ample whitespace following pattern, but, to do this we first need to remember
        // that
        // we fudged decoding CODE_STOP since it actually has 7 bars, not 6. There is a black bar
        // left
        // to read off. Would be slightly better to properly read. Here we just skip it:
        nextStart = row.getNextUnset(nextStart);
        if (!row.isRange(nextStart,
                Math.min(row.getSize(), nextStart + (nextStart - lastStart) / 2), false)) { throw NotFoundException
                .getNotFoundInstance(); }

        // Pull out from sum the value of the penultimate check code
        checksumTotal -= multiplier * lastCode;
        // lastCode is the checksum then:
        if (checksumTotal % 103 != lastCode) { throw ChecksumException.getChecksumInstance(); }

        // Need to pull out the check digits from string
        int resultLength = result.length();
        // Only bother if the result had at least one character, and if the checksum digit happened
        // to
        // be a printable character. If it was just interpreted as a control code, nothing to
        // remove.
