...
public abstract class OneDimensionalCodeWriter implements Writer {
    ...
    /**
     * Encode the contents following specified format. {@code width} and {@code height} are required
     * size. This method may return bigger size {@code BitMatrix} when specified size is too small.
     * The user can set both {@code width} and {@code height} to zero to get minimum size barcode.
     * If negative value is set to {@code width} or {@code height}, {@code IllegalArgumentException}
     * is thrown.
     */
    @Override
    public BitMatrix encode(String contents, BarcodeFormat format, int width, int height,
    ...
    protected static int appendPattern(byte[] target, int pos, int[] pattern, int startColor) {
        if (startColor != 0 && startColor != 1) { throw new IllegalArgumentException(
     ...
     * Start code and end code should be included in result, and side margins should not be
     * included.
     * 
     * @return a byte array of horizontal pixels (0 = white, 1 = black)
     */
