...
/**
 * <p>
 * Encapsulates functionality and implementation that is common to UPC and EAN families of
...
public abstract class UPCEANReader extends OneDReader {
    ...
    static int[] findStartGuardPattern(BitArray row) throws NotFoundException {
        ...
        while (!foundStart) {
            ...
            // Make sure there is a quiet zone at least as big as the start pattern before the
            // barcode.
    ...
    /**
     * <p>
     * Like {@link #decodeRow(int, BitArray, java.util.Map)}, but allows caller to inform method
     * about where the UPC/EAN start pattern is found. This allows this to be computed once and
     * reused across many implementations.
     * </p>
     */
    public Result decodeRow(int rowNumber, BitArray row, int[] startGuardRange,
        ...
        if (quietEnd >= row.getSize() || !row.isRange(end, quietEnd, false)) { throw NotFoundException
                .getNotFoundInstance(); }

        String resultString = result.toString();
        if (!checkChecksum(resultString)) { throw ChecksumException.getChecksumInstance(); }

        float left = (float) (startGuardRange[1] + startGuardRange[0]) / 2.0f;
        float right = (float) (endRange[1] + endRange[0]) / 2.0f;
        BarcodeFormat format = getBarcodeFormat();
        Result decodeResult = new Result(resultString, null, // no natural byte representation for
                                                             // these barcodes
                new ResultPoint[] {
    ...
    private static boolean checkStandardUPCEANChecksum(CharSequence s) throws FormatException {
        ...
        for (int i = length - 2; i >= 0; i -= 2) {
            ...
            if (digit < 0 || digit > 9) { throw FormatException.getFormatInstance(); }
            sum += digit;
        }
        sum *= 3;
        ...
        for (int x = rowOffset; x < width; x++) {
            if (row.get(x) ^ isWhite) {
                ...
                if (counterPosition == patternLength - 1) {
                    if (patternMatchVariance(counters, pattern, MAX_INDIVIDUAL_VARIANCE) < MAX_AVG_VARIANCE) { return new int[] {
                            patternStart, x
                    }; }
     ...
     *            for the digits 0-9 are used, and this indicates the encodings for 0 to 9 that
     *            should
     *            be used
