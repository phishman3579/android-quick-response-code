...
public final class Code39Reader extends OneDReader {
    ...
    public Result decodeRow(int rowNumber, BitArray row, Map<DecodeHintType, ?> hints)
        ...
        do {
            ...
            if (pattern < 0) { throw NotFoundException.getNotFoundInstance(); }
        ...
        if (nextStart != end && (whiteSpaceAfterEnd >> 1) < lastPatternSize) { throw NotFoundException
                .getNotFoundInstance(); }
        ...
        if (usingCheckDigit) {
            ...
            if (result.charAt(max) != ALPHABET[total % 43]) { throw ChecksumException
                    .getChecksumInstance(); }
            ...
            if (row.get(i) ^ isWhite) {
                ...
                if (counterPosition == patternLength - 1) {
                    if (toNarrowWidePattern(counters) == ASTERISK_ENCODING) {
                        ...
                        // pattern
                        if (row.isRange(Math.max(0, patternStart - ((i - patternStart) >> 1)),
        ...
        do {
            ...
            if (wideCounters == 3) {
                ...
                for (int i = 0; i < numCounters && wideCounters > 0; i++) {
                    ...
                    if (counters[i] > maxNarrowCounter) {
                        wideCounters--;
                        // totalWideCountersWidth = 3 * average, so this checks if counter >= 3/2 *
                        // average
                        if ((counter << 1) >= totalWideCountersWidth) { return -1; }
                    }
                }
                return pattern;
            }
