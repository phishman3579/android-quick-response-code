...
public final class CalendarParsedResult extends ParsedResult {
    ...
    public CalendarParsedResult(String summary, String start, String end, String location,
            String attendee, String description) {
        this(summary, start, end, location, attendee, description, Double.NaN, Double.NaN);
    }

    public CalendarParsedResult(String summary, String start, String end, String location,
            String attendee, String description, double latitude, double longitude) {
     ...
     * as {@code java.text.SimpleDateFormat}.
     * </p>
    ...
    private static void validateDate(CharSequence date) {
        if (date != null) {
            ...
            if (length != 8 && length != 15 && length != 16) { throw new IllegalArgumentException(); }
            for (int i = 0; i < 8; i++) {
                if (!Character.isDigit(date.charAt(i))) { throw new IllegalArgumentException(); }
            }
            if (length > 8) {
                if (date.charAt(8) != 'T') { throw new IllegalArgumentException(); }
                for (int i = 9; i < 15; i++) {
                    if (!Character.isDigit(date.charAt(i))) { throw new IllegalArgumentException(); }
                }
                if (length == 16 && date.charAt(15) != 'Z') { throw new IllegalArgumentException(); }
            }
        }
    }
