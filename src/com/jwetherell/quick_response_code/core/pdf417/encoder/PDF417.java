...
final class PDF417 {
    ...
    private void encodeLowLevel(CharSequence fullCodewords, int c, int r, int errorCorrectionLevel,
            BarcodeMatrix logic) {
        ...
        for (int y = 0; y < r; y++) {
            ...
            if (compact) {
                encodeChar(STOP_PATTERN, 1, logic.getCurrentRow()); // encodes stop line for compact
                                                                    // pdf417
            } else {
    ...
    void generateBarcodeLogic(String msg, int errorCorrectionLevel) throws WriterException {
        ...
        int errorCorrectionCodeWords = PDF417ErrorCorrection
                .getErrorCorrectionCodewordCount(errorCorrectionLevel);
        ...
        if (sourceCodeWords + errorCorrectionCodeWords + 1 > 929) { // +1 for symbol length CW
            throw new WriterException(
                    "Encoded message contains to many code words, message to big (" + msg.length()
                            + " bytes)");
        }
        ...
        String ec = PDF417ErrorCorrection.generateErrorCorrection(dataCodewords,
    ...
    int[] determineDimensions(int sourceCodeWords, int errorCorrectionCodeWords)
        ...
        if (dimension == null) { throw new WriterException("Unable to fit message in columns"); }
