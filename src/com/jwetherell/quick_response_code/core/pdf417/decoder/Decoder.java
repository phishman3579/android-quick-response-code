...
/**
 * <p>
 * The main class which implements PDF417 Code decoding -- as opposed to locating and extracting the
 * PDF417 Code from an image.
 * </p>
 * 
...
public final class Decoder {
    ...
    private static void verifyCodewordCount(int[] codewords, int numECCodewords)
        ...
        // The first codeword, the Symbol Length Descriptor, shall always encode the total number of
        // data
        // codewords in the symbol, including the Symbol Length Descriptor itself, data codewords
        // and pad
        // codewords, but excluding the number of error correction codewords.
        int numberOfCodewords = codewords[0];
        if (numberOfCodewords > codewords.length) { throw FormatException.getFormatInstance(); }
        if (numberOfCodewords == 0) {
            // Reset to the length of the array - 8 (Allow for at least level 3 Error Correction (8
            // Error Codewords)
    ...
    private static int correctErrors(int[] codewords, int[] erasures, int numECCodewords)
            throws FormatException {
        if (erasures.length > numECCodewords / 2 + MAX_ERRORS || numECCodewords < 0
