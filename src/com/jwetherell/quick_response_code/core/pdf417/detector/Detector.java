...
package com.jwetherell.quick_response_code.core.pdf417.detector;
...
/**
 * <p>
 * Encapsulates logic that can detect a PDF417 Code in an image, even if the PDF417 Code is rotated
...
public final class Detector {
    ...
    public DetectorResult detect(Map<DecodeHintType, ?> hints) throws NotFoundException {
        ...
        if (vertices == null) { throw NotFoundException.getNotFoundInstance(); }

        float moduleWidth = computeModuleWidth(vertices);
        if (moduleWidth < 1.0f) { throw NotFoundException.getNotFoundInstance(); }

        int dimension = computeDimension(vertices[4], vertices[6], vertices[5], vertices[7],
                moduleWidth);
        if (dimension < 1) { throw NotFoundException.getNotFoundInstance(); }

        // Deskew and sample image.
        BitMatrix bits = sampleGrid(matrix, vertices[4], vertices[5], vertices[6], vertices[7],
                dimension);
    ...
    private static ResultPoint[] findVertices180(BitMatrix matrix) {
        ...
        for (int i = height - 1; i > 0; i--) {
            int[] loc = findGuardPattern(matrix, halfWidth, i, halfWidth, true,
                    START_PATTERN_REVERSE, counters);
            if (loc != null) {
     ...
     * the codeword coordinates will be slightly wrong if there is any skew or rotation in the
     * image.
