...
package com.jwetherell.quick_response_code.core.datamatrix.decoder;
...
final class BitMatrixParser {
    ...
    BitMatrixParser(BitMatrix bitMatrix) throws FormatException {
        ...
        if (dimension < 8 || dimension > 144 || (dimension & 0x01) != 0) { throw FormatException
                .getFormatInstance(); }

        version = readVersion(bitMatrix);
        this.mappingBitMatrix = extractDataRegion(bitMatrix);
        this.readMappingMatrix = new BitMatrix(this.mappingBitMatrix.getWidth(),
    ...
    /**
     * <p>
     * Reads the bits in the {@link BitMatrix} representing the mapping matrix (No alignment
     * patterns) in the correct order in order to reconstitute the codewords bytes contained within
    ...
    byte[] readCodewords() throws FormatException {
        ...
        do {
            ...
            if ((row == numRows) && (column == 0) && !corner1Read) {
                ...
                column += 2;
                corner1Read = true;
            } else if ((row == numRows - 2) && (column == 0) && ((numColumns & 0x03) != 0)
                    && !corner2Read) {
                result[resultOffset++] = (byte) readCorner2(numRows, numColumns);
                row -= 2;
                column += 2;
                corner2Read = true;
            } else if ((row == numRows + 4) && (column == 2) && ((numColumns & 0x07) == 0)
                    && !corner3Read) {
                result[resultOffset++] = (byte) readCorner3(numRows, numColumns);
                row -= 2;
                column += 2;
                corner3Read = true;
            } else if ((row == numRows - 2) && (column == 0) && ((numColumns & 0x07) == 4)
                    && !corner4Read) {
     ...
     * @param column Current column in the mapping matrix, anchored at the 8th bit (LSB) of the
     *            pattern
     * @param numRows Number of rows in the mapping matrix
     * @param numColumns Number of columns in the mapping matrix
    ...
    BitMatrix extractDataRegion(BitMatrix bitMatrix) {
        ...
        if (bitMatrix.getHeight() != symbolSizeRows) { throw new IllegalArgumentException(
            ...
            for (int dataRegionColumn = 0; dataRegionColumn < numDataRegionsColumn; ++dataRegionColumn) {
                ...
                for (int i = 0; i < dataRegionSizeRows; ++i) {
                    ...
                    for (int j = 0; j < dataRegionSizeColumns; ++j) {
                        int readColumnOffset = dataRegionColumn * (dataRegionSizeColumns + 2) + 1
                                + j;
